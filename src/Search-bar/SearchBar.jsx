...
const SearchBar = ({ mode }) => {
  ...
  if (isError) {
    return <div className='py-4 mt-5 alert alert-danger'>Some Error Occurred. Try again later.</div>;
  }

  function handleFilter(e) {
    setViewDetailsBtn(false);
    let filtArr = products && products.filter(item => {
      const regex = new RegExp(`${e.target.value}`, "gi");
      if (mode === "Tag")
        return item.tag.match(regex);
  ...
  return (
    <div className='absolute-box'>
      <div>
        <input type='text' id='Input1' placeholder={mode === "Tag" ? "Enter Tag name" : mode === "Address" ? "Enter City or Address" : "Enter Product Name"} onChange={handleFilter} />
      </div>
      {viewDetailsBtn && <div className='viewDetailsBtn'>
      ...
      <div className={filteredList ? 'suggestions-alt ' : 'suggestions'}>
        {filteredList &&
          filteredList.map((item, index) => <div onClick={() => {
            setFilteredList([]);
            ...
            key={index} className='drop-down-list'>{
              mode === "Tag"
                ? (<h6>{item.name} <sub>{item.tag}</sub></h6>)
                : mode === "Address"
                  ? (<p>{item.name} <sub>{item.address}</sub></p>) : item.name
            }</div>)}
      </div>
    </div>
  )
